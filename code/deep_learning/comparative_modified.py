...
def within(painting, type, rater, origin=True, N=1, process_id=None):
    ...
    checkpoint_path = f"checkpoint/comparative_within_{process_id}.weights.h5"
...
def cross(painting, type, rater, origin=True, N=1, process_id=None):
    if painting == "abstract":
        if origin:
            X = np.load('feature/abstract_feature_origin.npy')
        else:
    ...
    if process_id is None:
        process_id = f"{os.getpid()}_{int(time.time()*1000000)}_{random.randint(0,999999)}"
    checkpoint_path = f"checkpoint/comparative_cross_{process_id}.weights.h5"
    os.makedirs(os.path.dirname(checkpoint_path), exist_ok=True)
